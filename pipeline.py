...
function _iburst(x,y,n,spd,cols,r=3,g=.08){{for(let i=0;i<n;i++){{const a=Math.random()*Math.PI*2,s=spd*(.4+Math.random()*.6),col=cols[Math.floor(Math.random()*cols.length)];_iparts.push({{x,y,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:1,decay:.011+Math.random()*.018,r:r*(.5+Math.random()*.5),col,grav:.06,tail:[]}});}}}}
function _iring(x,y,n,col,spd=16){{for(let i=0;i<n;i++){{const a=(Math.PI*2*i)/n;_iparts.push({{x,y,vx:Math.cos(a)*spd,vy:Math.sin(a)*spd,life:1,decay:.013+Math.random()*.01,r:3+Math.random()*3,col,grav:.025,tail:[]}});}}}}
...
let _iexploded=false,_ifo=0;
function _iframe(){{
  _it++;
  if(_ifo>0){{
    ictx.fillStyle=`rgba(0,0,0,${{_cl(_ifo,0,1)}})`;ictx.fillRect(0,0,IW,IH);
    _ifo+=.025;_tickp();_ticks();
    if(_ifo>1){{showSite();return;}}
